...
const DURATION = (__ENV.DURATION || '3m').trim(); // 최대 부하 유지 시간
...
function buildStages() {
  ...
  // 기본 단계: 25% → 50% → 75% → 100%
  return [
    { duration: '15s', target: Math.max(Math.floor(VUS * 0.25)) },
    { duration: '15s', target: Math.max(Math.floor(VUS * 0.5)) },
    { duration: '15s', target: Math.max(Math.floor(VUS * 0.75)) },
